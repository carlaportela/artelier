//Página que muestra el detalle de un pedido concreto.
...
import { CANCELLATION_WINDOW_MS } from "~/lib/order-constants";
...
export default async function OrderDetailPage({ params }: Props) {
  ...
  const order = await db.order.findFirst({
    ...
    include: {
      product: { select: { name: true, imageUrls: true, type: true, expiresAt: true } },
      ...
      <div className="mx-auto max-w-lg space-y-6">
        ...
        <div className="flex items-center gap-4 rounded-xl border border-[--border] bg-[--surface] p-4">
          {productImage ? (
            ...
            <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-lg bg-[--surface-2]">
              <Package className="h-7 w-7 text-[--text-muted]" strokeWidth={1.5} />
            </div>
          )}
          <div className="min-w-0">
            <p className="font-display font-bold text-[--text]">{order.product.name}</p>
        ...
        <div className="rounded-xl border border-[--border] bg-[--surface] p-4 space-y-2">
          <div className="flex justify-between text-sm">
            ...
            <span className="text-[--text]">{formatCents(order.priceInCents)}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-[--text-muted]">Gastos de envío y comisión</span>
          ...
          <div className="border-t border-[--border] pt-2 flex justify-between text-sm font-semibold">
            ...
            <span className="text-[--text]">{formatCents(order.totalInCents)}</span>
          </div>
        </div>
        ...
        {order.cancellationReason && (
          <div className="rounded-xl border border-red-100 bg-red-50 p-4">
            <p className="text-xs font-medium text-red-600">Motivo de cancelación</p>
