...
- [x] T5 — Componentes de timeline (AC1, AC2, AC3)
  - [x] T5.1: Crear `src/components/order/OrderStatusTimeline.tsx` — recibe `status: OrderStatus` y `shippingMethod: ShippingMethod`. Renderiza un `<ol>` con los 6 estados canónicos. El paso actual lleva `aria-current="step"`. Si `shippingMethod === "PICKUP"`, omite visualmente el paso "Enviado" y sustituye la etiqueta de "Listo" por la traducción `orderStatus.READY_PICKUP` (nueva clave añadida a `es.json` — corregido durante la implementación: la idea original de hardcodear el texto en el componente rompía el sistema de traducciones, ver Dev Notes)
  - [x] T5.2: Crear `src/components/order/OrderStatusUpdate.tsx` — recibe `status`, `message`, `createdAt` de un `OrderStatusUpdate`; renderiza una entrada de timeline con el mensaje personal si existe (arquitecture.md original lo ubicaba en `components/messaging/`, pero no reutiliza el modelo `Message` — se coloca en `components/order/` por cohesión con el dato que realmente representa; no hay `components/messaging/` en el código actual, así que no hay convención que romper)
  - [x] T5.3: Crear `src/app/(artisan)/studio/orders/[id]/AdvanceStatusForm.tsx` (mismo patrón `"use client"`, fetch + toast + `router.refresh()` que el resto de esta zona) — muestra el campo de `trackingNumber` solo cuando el siguiente paso es `SHIPPED` (`order.status === "READY" && shippingMethod !== "PICKUP"`), y siempre un textarea opcional de mensaje personal con contador de caracteres (mismo patrón que `AcceptOrRejectOrderCard.tsx`, mínimo 0 en vez de 10, máximo `ORDER_STATUS_UPDATE_MESSAGE_MAX_LENGTH`). Todo su texto pasa por `useTranslations("account")` — ver T5.7. `ConfirmShipmentForm.tsx` se elimina en T5.4, cuando `page.tsx` deja de importarlo
...
La clave de traducción `orderStatus.READY` en `es.json` ("Listo para enviar") sirve como etiqueta genérica plana (p.ej. la pastilla de estado que ya existe arriba de la página de detalle, sin tocar). El componente `OrderStatusTimeline` (y `OrderStatusUpdate`), en cambio, necesitan distinguir contextualmente: si `shippingMethod === "PICKUP"`, el paso equivalente a `READY` debe leerse "Listo para recogida" (coherente con el asunto ya existente del email `OrderReadyForPickupEmail`: "Tu pedido está listo para recoger"); si no, "Listo para enviar" tal cual.

**Corrección durante la implementación:** la primera versión de esta nota decía que el texto "Listo para recogida" debía vivir hardcodeado dentro del componente, sin tocar `es.json`, razonando que era "presentación local". Eso estaba mal — cualquier texto visible para la usuaria debe pasar por el sistema de traducciones (`t(...)`), sin excepción, o se rompe la i18n de la app (un texto hardcodeado nunca cambiaría de idioma aunque el resto de la página sí lo hiciera, y quien traduzca la app no vería ese texto en ningún JSON para traducirlo). Se añadió la clave `orderStatus.READY_PICKUP` a `es.json`, y ambos componentes la consultan con `t("orderStatus.READY_PICKUP")` en vez de un string literal.
